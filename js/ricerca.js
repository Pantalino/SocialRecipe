document.addEventListener("DOMContentLoaded", function () {
    ...
    const utenteID = urlParams.get('utenteID');
 
    fetch(`http://localhost:8080/api/utenti/${utenteID}/ricette`)
    ...
    .then(data => {
            ...
            data.forEach(ricetta => {
                ...
                fetch(`http://localhost:8080/api/likes/${ricetta.id}/utente/${utenteID}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.length > 0) {
                ...
                submitButton.addEventListener('click', function () {
                    ...
                    function searchUser(query) {
                        ...
                        fetch(`http://localhost:8080/api/utenti/search/by-nome?nome=${query}`)
                            ...
                            // fetch(`http://localhost:8080/api/utenti/search/by-nome-cognome?nome-cognome=${formattedQuery}`)
                            //     .then(response => response.json())
                            //     .then(data => displayResults(data))
                            //     .catch(error => console.error('Errore durante la ricerca:', error));
